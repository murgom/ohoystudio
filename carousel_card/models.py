...
class SampleCarouselImage(models.Model):
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT)
    title = models.CharField(max_length=100)
    ohoystudio_url = models.CharField(max_length=300)
    youtube_url = models.TextField(default='')
    ...
    class Meta:
        verbose_name = '(샘플)이미지 스와이프 청첩장'
        verbose_name_plural = '(샘플)이미지 스와이프 청첩장(메인)'

    def __str__(self):
        return self.title

    def get_absolute_url(self):
        return reverse('carousel_card:sample_post_detail',args=[self.id])

class SampleCarouselGalleryImage(models.Model):
    post = models.ForeignKey(SampleCarouselImage, on_delete=models.PROTECT)
    name = models.CharField(max_length=100,blank=True)
    ...
    class Meta:
        verbose_name = '(샘플)갤러리 이미지'
        verbose_name_plural = '(샘플)갤러리 이미지'

class SampleCarouselSmsImage(models.Model):
    post = models.ForeignKey(SampleCarouselImage, on_delete=models.PROTECT)
    name = models.CharField(max_length=100)
    message_image1 = ProcessedImageField(blank=True,
            processors=[Thumbnail(2000, 2000)],
