...
class SampleInfomationImage(models.Model):
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT)
    title = models.CharField(max_length=100)
    ohoystudio_url = models.CharField(max_length=300)
    youtube_url = models.TextField(default='')
    ...
    class Meta:
        verbose_name = '(샘플)댓글없는 청첩장'
        verbose_name_plural = '(샘플)댓글없는 청첩장(메인)'

    def __str__(self):
        return self.title

    def get_absolute_url(self):
        return reverse('card2:sample_post_detail',args=[self.id])

class SampleGalleryImage(models.Model):
    post = models.ForeignKey(SampleInfomationImage, on_delete=models.PROTECT)
    name = models.CharField(max_length=100)
    gallery_image1 = ProcessedImageField(blank=True,
            processors=[Thumbnail(2000, 2000)],
            upload_to=get_image_path,
